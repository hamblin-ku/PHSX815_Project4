...
class Random:
    ...
    # Simulates a spin measurement of a fermion, returns 0 for (-1/2) sping and 1 for (+1/2) spin
    # The bias value specifies the probability of a down spin state (0.5 by default)
    def fermion(self, bias = 0.5):
        """Simulates a spin measurement of a fermion with optional bias"""
        roll = self.roll_die(Nsides = 2, weights = [bias, 1 - bias])
        if roll == 1:
            return 0
        else:
            return 1
